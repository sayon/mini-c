...
Section EqValue.
  Inductive value : Set := | Value (t:ctype) (v: coq_type t) | Garbage | Deallocated | Error.
  ...
  Lemma value_eq_dec (x y: value) : { x = y } + { ~ x = y }.
    case x; case y; try by [right|left].
    clear  x y.
    move=> tx x ty y.
    move:  (ctype_eq_dec tx ty); case=> H.
    - subst. move: (coq_type_eq_dec _ x y); case=> H.
      + by subst; left.
      + by right; case => H'; symmetry in H'; depcomp H'.
    - by right; case=> H'; symmetry in H'.
  Qed.
  
  Definition value_eqP := reflect_from_dec value_eq_dec.
